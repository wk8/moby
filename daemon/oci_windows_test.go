...
func TestSetWindowsCredentialSpecInSpec(t *testing.T) {
	...
	t.Run("happy path with a 'base64://' option", func(t *testing.T) {
		spec := &specs.Spec{}

		encoded := base64.StdEncoding.EncodeToString([]byte(dummyCredFileContents))
		err := daemon.setWindowsCredentialSpec(containerFactory("base64://"+encoded), spec)
		assert.NilError(t, err)
	...
	t.Run("when using a 'base64://' option with a non-base64 encoded value, it fails gracefully", func(t *testing.T) {
		spec := &specs.Spec{}

		bogusB64String := "af0*x"
		err := daemon.setWindowsCredentialSpec(containerFactory("base64://"+bogusB64String), spec)
		assert.ErrorContains(t, err, fmt.Sprintf("provided value %q is not a base64-encoded string", bogusB64String))
	...
	t.Run("it's not case sensitive in the option names", func(t *testing.T) {
		...
		encoded := base64.StdEncoding.EncodeToString([]byte(dummyCredFileContents))
		err := daemon.setWindowsCredentialSpec(containerFactory("CreDENtiaLSPeC=bASe64://"+encoded), spec)
		assert.NilError(t, err)
	...
	for _, option := range []string{"file", "registry", "config", "base64"} {
